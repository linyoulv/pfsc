...
<body>
	...
	<div class="text-c">
			...
			<input type="text" value="${m_username}" 
					id="m_username"  class="input-text"
					style="width: 55%;" placeholder="请输入商户名称">
			...
			<script type="text/javascript">
				function search() {
				    ...
				    window.location.href = 'merchantList.html?m_username=' + encodeURIComponent(encodeURIComponent(m_username));
				}
			</script>
		</div>
	<div class="pd-20">
	<!-- 	<div class="cl pd-5 bg-1 bk-gray mt-20">
		...
		<div class="mt-20">
				...
				<tbody>
					<c:forEach items="${goods}" var="list" varStatus="s">
						<tr class="text-c">
							...
							<td>${list.m_repassword}</td>
							<td>${list.m_address}</td>
							<td>${list.m_phone}</td>
							
					  	  <td>
					  	    <c:if test="${list.m_shbz==0}">
							<a href="javascript:;" onclick="sh('${list.id}')" ><font color="red">审核</font></a>&nbsp;&nbsp;
							</c:if>
							
							  <c:if test="${list.m_shbz==1}">
							<p>已审核</p>&nbsp;&nbsp;
							</c:if>
						   <c:if test="${list.m_shbz==0}">
			...
			<div class="panel-foot text-center">
				...
				<ul class="pagination">
					<li><a href="javascript:;" onclick="firstPage()">首页</a></li>
				</ul>
				<ul class="pagination">
				...
				<ul class="pagination">
					<li><a href="javascript:;" onclick="endPage()">末页</a></li>
				</ul>
	...
	function upPage() {
		if (currentPage > 1) {
			window.location.href = 'merchantList.html?currentPage='
	...
	function firstPage() {
		window.location.href = 'merchantList.html?currentPage=1';
		return;
	}
	function endPage() {
		window.location.href = 'merchantList.html?currentPage=' + totalPage;
		return;
	}
	function spage() {
		var seastr =$('#seastr').val();
		if (parseInt(seastr)< parseInt(totalPage)||parseInt(seastr)== parseInt(totalPage)) {
		window.location.href = 'merchantList.html?currentPage=' + seastr;
		}
		return;
	}
	</script>
