...
<body style="/* width: 1720px */">
	<nav class="breadcrumb">
		...
		跑腿订单查看 <a class="btn btn-success radius r mr-20"
...
…….get(0).selectedIndex; -->
			...
			<script type="text/javascript">
				function search() {
					...
					window.location.href = 'paotuiorderList.html?start_time=' + encodeURIComponent(encodeURIComponent(start_time))
				...
				<tbody>
					...
					<c:set value="ord${s.index}" var="ord"></c:set>
						...
						<br>
							...
							<td class="text-c">
							<c:if test="${list.paystate==0 }">否</c:if>
							<c:if test="${list.paystate==1 }">是</c:if>
							</td>
							
							<td>${list.order_where}</td>
							<td>${list.order_what}</td>
