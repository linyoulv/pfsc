...
<body>
	<nav class="breadcrumb">
		...
		跑腿费 <span class="c-gray en">&gt;</span>  <a
	...
	<div class="pd-20" style="width: 80%">
			...
			<div class="row cl"><input id="id" type="hidden" value="${list.id }">
						<label class="form-label col-2">${list.errands_name}</label>
						<div class="formControls col-10">
							<input type="text" id="errands_price"
								placeholder="请填写" value="${list.errands_price}" class="input-text" style="width: 80%">
						</div>
			</div><br>
	...
	function add(){
		var id = $('#id').val();
		var errands_price = $('#errands_price').val();
		 
		$.ajax({
			url:'errandsUpdate.html',
			type:'post',
			data:'errands_price='+errands_price+'&id='+id,
			success:function(rs){
				if(rs==1){
